...
execute as @e[tag=SnowballAttack] at @s run particle dust{color:[0.788,0.953,1.000],scale:2} ~ ~ ~ 0 0 0 1 0 force
...
execute as @e[type=minecraft:zombie,tag=!changeDone] at @s if biome ~ ~ ~ #minecraft:is_jungle run function vanilla_plus:jungle/poisoning
## Jungle Zombie's particle
# execute as @e[predicate=vanilla_plus:jungle_poisonous_mobs] at @s run particle dust{color:[0.012,0.220,0.067],scale:2} ~ ~1 ~ 0.25 0.25 0.25 3 1 force
...
execute as @e[type=minecraft:cow,tag=!vpWooledCow] at @s if biome ~ ~ ~ #vanilla_plus:replace_with_wooly_cow run function vanilla_plus:coniferous_forest/replace_wooly
...
# execute as @e[type=#knf:soul_sand_nether_fortress_monsters,tag=!vpFortressSpawned,predicate=vanilla_plus:in_nether,tag=vpNetherSpawned] at @s run function knf:soul_sand_valley/soul_sand_nether_fortress_monster_check

# execute as @e[type=minecraft:wither_skeleton,tag=vpFortressSpawned,tag=knfFromSpawner,tag=!knfModified] run function knf:soul_sand_valley/wither_skeleton_from_spawner_mofify

execute as @e[type=#knf:soul_sand_nether_fortress_monsters,nbt={active_effects:[{id:"minecraft:wither"}]},tag=vpFortressSpawned] at @s run effect clear @s minecraft:wither

##Fire Trails Blaze
# execute as @e[type=minecraft:blaze,tag=vpFortressSpawned,tag=knf_fire_trails] at @s run fill ~ ~ ~ ~ ~ ~ minecraft:fire keep
...
# in Trial Chambers
## Bursting Miasma
# execute as @e[type=minecraft:bogged,tag=vpBurstingMiasma,tag=!vpMiasmaRided] at @s run function vanilla_plus:trial_chambers/bursting_miasma/summon_zombie_horse
# execute as @e[tag=vpBurstingMiasmaHorse] at @s unless data entity @s Passengers[{id:"minecraft:bogged"}] run effect clear @s minecraft:instant_damage
